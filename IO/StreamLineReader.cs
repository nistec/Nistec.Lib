...
namespace Nistec.IO
{
	...
	public class StreamLineReader
	{
		...
		/// <summary>
        /// Initialize a new instance of <see cref="StreamReader"/>.
		/// </summary>
        ...
        public byte[] ReadBlock(int length)
        {
            ...
            if (m_StrmSource.Position + length >= m_StrmSource.Length)
            {
                return null;
            }
            byte[] b = new byte[length];
            m_StrmSource.Read(b, 0, length);
            //Buffer.BlockCopy(m_Buffer, offset, b, 0, length);
            return b;
        }
